...
RUNTIME_VERSION="qwen-image-2.1-v2"
...
PROMPT_ENHANCER_T2I_ID="${PROMPT_ENHANCER_T2I_ID:-Qwen/Qwen-Image-2.1-PE-T2I}"
PROMPT_ENHANCER_I2I_ID="${PROMPT_ENHANCER_I2I_ID:-Qwen/Qwen-Image-2.1-PE-I2I}"
PROMPT_ENHANCER_MAX_NEW_TOKENS="${PROMPT_ENHANCER_MAX_NEW_TOKENS:-1024}"
PROMPT_ENHANCER_FAIL_OPEN="${PROMPT_ENHANCER_FAIL_OPEN:-true}"
...
if [[ -r /proc/1/environ ]]; then
  while IFS= read -r -d '' entry; do
    key="${entry%%=*}"
    case "$key" in
      HF_TOKEN|PIXELPILOT_INFERENCE_TOKEN|INFERENCE_PORT|MODEL_ID|MODEL_DTYPE|IMAGE_MEMORY_MODE|IMAGE_FULL_GPU_MIN_VRAM_GB|IMAGE_VAE_TILING|IMAGE_VAE_SLICING|IMAGE_MAX_REFERENCE_IMAGES|IMAGE_MAX_UPLOAD_MB|PROMPT_ENHANCER_T2I_ID|PROMPT_ENHANCER_I2I_ID|PROMPT_ENHANCER_MAX_NEW_TOKENS|PROMPT_ENHANCER_FAIL_OPEN|HF_HOME|DATA_DIRECTORY|PIXELPILOT_REPO_URL|PIXELPILOT_REPO_REF)
...
export PROMPT_ENHANCER_T2I_ID PROMPT_ENHANCER_I2I_ID PROMPT_ENHANCER_MAX_NEW_TOKENS PROMPT_ENHANCER_FAIL_OPEN
