...
PROFILE = {
    ...
    "MODEL_GPU_MEMORY_UTILIZATION": "0.92",
...
def update_env_text(text: str) -> str:
    ...
    if missing:
        if output and output[-1] != "":
            output.append("")
        output.append("# PixelPilot economical Qwen2.5-Omni-7B profile")
...
def migrate(path: Path) -> Path:
    ...
    backup = path.with_name(f"{path.name}.before_qwen25_7b_{stamp}.bak")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Switch an existing PixelPilot .env to the economical Qwen2.5-Omni-7B profile without touching secrets."
    )
