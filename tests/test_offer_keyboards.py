...
def test_identical_offer_specs_show_distinct_ids():
    offers = [
        GpuOffer(21004761, "RTX A6000", 49, 0.428, 0.989, 40),
        GpuOffer(21004888, "RTX A6000", 49, 0.428, 0.989, 40),
    ]
    markup = offers_keyboard(offers)
    first, second = markup.inline_keyboard[:2]
    assert "#21004761" in first[0].text
    assert "#21004888" in second[0].text
    assert first[0].callback_data != second[0].callback_data
